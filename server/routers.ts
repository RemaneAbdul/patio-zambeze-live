...
import { quickWaiterLogin, generateWaiterCode, assignNewWaiterCode, ensureNumericWaiterCodes } from "./quickWaiterLogin";
...
export const appRouter = router({
  ...
  staff: router({
    ...
    quickLogin: publicProcedure.input(z.object({ code: z.string().regex(/^\d{6}$/) })).mutation(async ({ input, ctx }) => { try { const result = await quickWaiterLogin(input.code, ctx.req.ip || ctx.req.socket.remoteAddress || "unknown"); ctx.res.cookie(COOKIE_NAME, result.sessionToken, getSessionCookieOptions(ctx.req)); await recordAuditLog({ userId: result.waiter.id, role: "garcom", action: "AUTH_QUICK_LOGIN_SUCCESS", entityType: "auth_session", entityId: result.sessionToken.slice(0, 16) }); return { waiter: result.waiter }; } catch (error) { const message = error instanceof Error ? error.message : "WAITER_CODE_INVALID"; if (message === "WAITER_LOGIN_RATE_LIMITED") throw new Error("Muitas tentativas de login. Aguarde alguns segundos antes de tentar novamente."); throw new Error("WAITER_CODE_INVALID"); } }),
