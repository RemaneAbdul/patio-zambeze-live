...
describe("tableHistory validation", () => {
  ...
  it("allows an active waiter to consult QR codes but not generate them", async () => {
    const caller = appRouter.createCaller({ ...ctx, user: { id: 42, openId: "waiter-user", email: "waiter@example.com", name: "Waiter User", loginMethod: "manus", role: "garcom", waiterCode: "123456", waiterActive: 1, createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() } });
