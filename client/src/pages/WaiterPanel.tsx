        ...
        {manualOpen && (
          <section ref={manualOrderRef} className="waiter-manual-order" aria-labelledby="manual-order-title">
            ...
            <form onSubmit={submitManualOrder} className="manual-order-form">
              ...
              <aside className="manual-order-current" aria-labelledby="manual-current-title"><div className="manual-order-current-heading"><div><p className="eyebrow">5 · Pedido actual</p><h3 id="manual-current-title">Pedido actual</h3><p>{manualTableId ? <>Mesa {qrCodes.data?.find((table) => table.qrToken === manualTableId)?.tableNumber ?? "seleccionada"}</> : "Seleccione uma mesa para começar"}</p></div><span className="manual-order-count">{manualItems.reduce((sum, item) => sum + item.quantity, 0)} itens</span></div>{selectedManualItems.length ? <div className="manual-order-current-items">{selectedManualItems.map((item) => <article key={item.productId} className="manual-current-item"><div className="manual-current-item-info"><strong>{item.product?.name ?? "Prato indisponível"}</strong><span>{money(Number(item.product?.price ?? 0))} cada</span></div><div className="manual-current-item-actions"><Button type="button" size="sm" variant="outline" onClick={() => setManualItems((current) => current.map((entry) => entry.productId === item.productId ? { ...entry, quantity: Math.max(1, entry.quantity - 1) } : entry))} aria-label={"Diminuir quantidade de " + (item.product?.name ?? "prato")}>−</Button><strong>{item.quantity}</strong><Button type="button" size="sm" variant="outline" onClick={() => setManualItems((current) => current.map((entry) => entry.productId === item.productId ? { ...entry, quantity: entry.quantity + 1 } : entry))} aria-label={"Aumentar quantidade de " + (item.product?.name ?? "prato")}>+</Button><Button type="button" size="sm" variant="destructive" className="manual-remove-item" onClick={() => setManualItems((current) => current.filter((entry) => entry.productId !== item.productId))} aria-label={"Remover " + (item.product?.name ?? "prato") + " do pedido"}>Remover</Button></div></article>)}</div> : <div className="manual-order-current-empty"><span>O pedido ainda está vazio.</span><small>Adicione pratos a partir do catálogo.</small></div>}<label className="manual-order-notes">Observação <span>(opcional)</span><textarea value={manualNotes} maxLength={1000} onChange={(event) => setManualNotes(event.target.value)} placeholder="Ex.: sem cebola, pouco sal…" /></label><div className="manual-order-total"><span>Total</span><strong>{money(selectedManualItems.reduce((sum, item) => sum + Number(item.product?.price ?? 0) * item.quantity, 0))}</strong></div><div className="manual-order-confirm"><Button type="submit" disabled={createManualOrder.isPending || !manualTableId || !manualItems.length} aria-busy={createManualOrder.isPending}>{createManualOrder.isPending ? <><LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> A criar pedido…</> : <><CheckCircle2 className="h-4 w-4" /> Confirmar pedido</>}</Button>{manualError && <p className="waiter-alert" role="alert">{manualError}</p>}</div></aside>
            </form>
          </section>
        )}
