...
export default function WaiterPanel() {
  ...
  const menuCatalog = trpc.menu.active.useQuery(undefined, { enabled: isAuthorized, retry: false });
  ...
  const manualCategories = useMemo(() => Array.from(new Map((menuCatalog.data ?? []).map((entry) => [entry.category.id, entry.category])).values()), [menuCatalog.data]);
  const manualProducts = useMemo(() => (menuCatalog.data ?? []).filter((entry) => !manualCategory || entry.category.id === Number(manualCategory)), [manualCategory, menuCatalog.data]);
  ...
  const addManualItem = () => { const productId = Number(manualProductId); if (!productId) return; setManualItems((current) => { const existing = current.find((item) => item.productId === productId); return existing ? current.map((item) => item.productId === productId ? { ...item, quantity: item.quantity + 1 } : item) : [...current, { productId, quantity: 1 }]; }); setManualProductId(""); };
