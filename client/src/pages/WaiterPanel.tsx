...
export default function WaiterPanel() {
  ...
  const createManualOrder = trpc.tableHistory.createManualOrder.useMutation({ onSuccess: async () => { setManualSuccess(true); window.setTimeout(() => setManualSuccess(false), 3600); setManualOpen(false); setManualFiltersOpen(false); setManualTableId(""); setManualCategory(""); setManualSearch(""); setManualProductId(""); setManualItems([]); setManualNotes(""); await Promise.all([utils.tableHistory.staffTables.invalidate(), qrCodes.refetch()]); } });
  ...
  const selectedManualProduct = useMemo(() => (menuCatalog.data ?? []).find((entry) => entry.product.id === Number(manualProductId)), [manualProductId, menuCatalog.data]);
  ...
  const addManualItem = () => { const productId = Number(manualProductId); if (!productId || selectedManualProduct?.product.status !== "ACTIVE") return; setManualItems((current) => { const existing = current.find((item) => item.productId === productId); return existing ? current.map((item) => item.productId === productId ? { ...item, quantity: item.quantity + 1 } : item) : [...current, { productId, quantity: 1 }]; }); setManualProductId(""); };
  ...
  return (
    <DashboardLayout>
      <div className="waiter-shell">
        ...
        {manualOpen && <section ref={manualOrderRef} className="waiter-manual-order" aria-labelledby="manual-order-title"><div className="waiter-section-heading"><div><p className="eyebrow">Atendimento presencial</p><h2 id="manual-order-title">Novo Pedido (pedido feito pelo cliente)</h2><p>Registe o pedido do cliente na mesma mesa e no mesmo catálogo do menu digital.</p></div><Button type="button" variant="outline" onClick={() => { setManualFiltersOpen(false); setManualOpen(false); }}><X className="h-4 w-4" /> Cancelar</Button></div><form onSubmit={submitManualOrder} className="waiter-manual-order-form"><div className="sm:col-span-2 waiter-manual-filter-toolbar"><Button type="button" variant="outline" className="manual-catalog-search-button" onClick={() => setManualFiltersOpen((current) => !current)} aria-expanded={manualFiltersOpen} aria-label={manualFiltersOpen ? "Ocultar filtros do catálogo" : "Pesquisar no catálogo"}><Search className="h-4 w-4" aria-hidden="true" /> <span>{manualFiltersOpen ? "Ocultar filtros" : "Pesquisar no catálogo"}</span></Button>{(manualSearch || manualCategory) && <Button type="button" variant="ghost" onClick={() => { setManualSearch(""); setManualCategory(""); setManualProductId(""); }}>Limpar filtros</Button>}</div>{manualFiltersOpen && <><label className="sm:col-span-2">Pesquisar no catálogo<input value={manualSearch} onChange={(event) => setManualSearch(event.target.value)} placeholder="Pesquisar prato…" /></label><label>Categoria<select value={manualCategory} onChange={(event) => { setManualCategory(event.target.value); setManualProductId(""); }}><option value="">Todas as categorias</option>{manualCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label></>}<label>Mesa<select required value={manualTableId} onChange={(event) => setManualTableId(event.target.value)}><option value="">Seleccione a mesa</option>{(qrCodes.data ?? []).map((table) => <option key={table.qrToken} value={table.qrToken}>Mesa {table.tableNumber}</option>)}</select></label><label>Prato<select value={manualProductId} onChange={(event) => setManualProductId(event.target.value)}><option value="">Seleccione o prato</option>{manualProducts.map((entry) => <option key={entry.product.id} value={entry.product.id} disabled={entry.product.status !== "ACTIVE"}>{entry.product.name} · {money(Number(entry.product.price))}{entry.product.status !== "ACTIVE" ? " · Indisponível" : ""}</option>)}</select></label><Button type="button" variant="outline" onClick={addManualItem} disabled={!manualProductId || selectedManualProduct?.product.status !== "ACTIVE"}><Plus className="h-4 w-4" /> Adicionar</Button><label className="sm:col-span-2">Observação <span className="text-xs text-muted-foreground">(opcional)</span><textarea value={manualNotes} maxLength={1000} onChange={(event) => setManualNotes(event.target.value)} placeholder="Ex.: sem cebola, pouco sal…" className="min-h-20 w-full rounded-md border border-border bg-background p-2" /></label><div className="sm:col-span-2">{selectedManualItems.length ? <div className="space-y-2">{selectedManualItems.map((item) => <div key={item.productId} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-background p-3"><span>{item.product?.name ?? "Prato indisponível"} <small>· {money(Number(item.product?.price ?? 0))}</small></span><div className="flex items-center gap-2"><Button type="button" size="sm" variant="outline" onClick={() => setManualItems((current) => current.map((entry) => entry.productId === item.productId ? { ...entry, quantity: Math.max(1, entry.quantity - 1) } : entry))}>−</Button><strong>{item.quantity}</strong><Button type="button" size="sm" variant="outline" onClick={() => setManualItems((current) => current.map((entry) => entry.productId === item.productId ? { ...entry, quantity: entry.quantity + 1 } : entry))}>+</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={() => setManualItems((current) => current.filter((entry) => entry.productId !== item.productId))}>Remover</Button></div></div>)}</div> : <p className="text-sm text-muted-foreground">Adicione pelo menos um prato ao pedido.</p>}</div><div className="flex flex-wrap items-center gap-3 sm:col-span-2"><Button type="submit" disabled={createManualOrder.isPending || !manualTableId || !manualItems.length} aria-busy={createManualOrder.isPending}>{createManualOrder.isPending ? <><LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> A criar pedido…</> : "Confirmar Pedido"}</Button>{manualError && <p className="waiter-alert" role="alert">{manualError}</p>}</div></form></section>}
