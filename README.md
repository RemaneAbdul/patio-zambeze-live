...
Antes de apontar tráfego real para Vercel, execute `pnpm exec tsc --noEmit`, `pnpm test` e `pnpm build`. O teste `server/supabaseConnection.test.ts` verifica a ligação com `SELECT 1`, enquanto `server/tableHistory.test.ts` cobre criação idempotente de sessões, pedidos separados, atendimento exclusivo, marcação como visto, QR permanente e limpeza controlada. O schema remoto foi verificado sem apagar dados durante esta migração.
